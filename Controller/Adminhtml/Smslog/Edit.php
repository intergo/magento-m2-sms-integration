...
class Edit extends \Smsto\Sms\Controller\Adminhtml\Smslog
{
    ...
    public function execute()
    {
        ...
        $this->_coreRegistry->register('smsto_smslog', $model);

        // 3. Build edit form
        /** @var \Magento\Backend\Model\View\Result\Page $resultPage */
        $resultPage = $this->resultPageFactory->create();
        $this->initPage($resultPage)->addBreadcrumb(
            $id ? __('Edit Smslog') : __('Send SMS'),
            $id ? __('Edit Smslog') : __('Send SMS')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('SMS Log History'));
        $resultPage->getConfig()->getTitle()->prepend($model->getId() ? __('Edit Smslog %1', $model->getId()) : __('Send SMS'));
        return $resultPage;
    }
}
